...
const TodoApp: TodoApp = () => {
    const [store, {addTodo, toggleAll, editTodo, removeTodo, clearCompleted, setVisibility}] = createTodosStore(),
        locationHandler = () => setVisibility(location.hash.slice(2) as ShowMode || 'all');
...
const TodoHeader = ({addTodo}: Pick<Actions, 'addTodo'>) =>
    <header class='header'>
        <h1>todos</h1>
        <input class='new-todo' placeholder='What needs to be done?' onKeyUp={({target, code}: KeyboardEvent) => {
...
const TodoList = ({store, editTodo, removeTodo, toggleAll}: ListProps) => {
    const [state, setState] = createState({} as ListState),
        ...
        isEditing = (todoId: number) => state.editingTodoId === todoId,
        ...
        toggle = (todoId: number, completed: boolean) => editTodo({id: todoId, completed}),
        edit = (todoId: number) => setCurrent(todoId),
    ...
    return <section class='main'>
        ...
        <ul class='todo-list'>
            <For each={(filterList(store.todos))} transform={selectWhen(() => state.editingTodoId, 'editing')}>
                {
                    todo => <TodoItem {...{todo, isEditing, toggle, edit, remove, setCurrent, save}} />
                }
            </For>
        </ul>
    </section>
...
const TodoItem = ({todo, isEditing, toggle, edit, remove, setCurrent, save}: { todo: Todo } & ListActions) => {
    const doneEdit = (e: KeyboardEvent) => {
        const input = e.target as HTMLInputElement;
        if (e.code === "Enter") save(todo.id, input.value.trim());
        else if (e.code === "Escape") setCurrent();
    };
    return <li class='todo' classList={({completed: todo.completed})}>
        <div class='view'>
            <input class='toggle'
                   type='checkbox'
        ...
        <Show when={(isEditing(todo.id))}>
            <input class='edit' value={todo.title} onBlur={(e: Event) => {
                const input = e.target as HTMLInputElement;
                save(todo.id, input.value.trim())
            }} onkeyup={doneEdit} forwardRef={setFocus}/>
        </Show>
    </li>
}

const TodoFooter = ({store, clearCompleted}: { store: Store } & Pick<Actions, 'clearCompleted'>) =>
    <footer class='footer'>
    <span class='todo-count'>
      <strong>{(store.remainingCount)}</strong>
        ...
        <Show when={(store.completedCount > 0)}
              children={<button class='clear-completed' onClick={clearCompleted}>Clear completed</button>}/>
    </footer>
