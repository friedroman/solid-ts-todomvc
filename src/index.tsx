...
const TodoApp: TodoApp = () => {
  ...
  const locationHandler = () =>
    setVisibility((location.hash.slice(2) as ShowMode) || "all");
...
type ListProps = StoreHolder &
  Pick<Actions, "editTodo" | "removeTodo" | "toggleAll">;
    ...
    <section class="main">
      <input id="toggle-all"
             ...
             onInput={({target}: Event) =>
               toggleAll((target as HTMLInputElement).checked)
             }/>
      ...
      <ul class="todo-list">
        <For each={filterList(store.todos)} transform={selectWhen(() => state.editingTodoId, "editing")}>
          {todo => (
            <TodoItem
              {...{todo, isEditing, toggle, remove, setCurrent, save}}
            />
          )}
        </For>
      </ul>
...
const TodoItem = ({ todo, isEditing, toggle, remove, setCurrent, save }: { todo: Todo } & ListActions) => {
  ...
  return (
    <li class="todo" classList={{completed: todo.completed}}>
      <div class="view">
        <input class="toggle" type="checkbox" checked={todo.completed} onInput={(e: Event) => {
